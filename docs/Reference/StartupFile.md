...
Once the class library is finished compiling the interpreter looks for a file at `Platform.userConfigDir +/+ "startup.scd"` and if such a file exists, executes any code within it (this happens within [Main#-startup](../Classes/Main.md#-startup)). By creating a file in this location you can make user specific customizations.
