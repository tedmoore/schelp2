...
**Related:** [Server](../Classes/Server.md), [Server-Command-Reference](../Reference/Server-Command-Reference.md), [AudioDeviceSelection](../Reference/AudioDeviceSelection.md)
...
: name of the hardware I/O device. If not provided, the default device is used. See [AudioDeviceSelection](../Reference/AudioDeviceSelection.md) for more details.
...
: Run in non-real-time mode.The cmd-filename should be a file that contains OSC bundles sorted in ascending time order. If cmd-filename is the underscore character _, then OSC will be streamed from standard input.The audio input will taken from input-filename. If input-filename is the underscore character _, then no input file will be read.Output will be written to output-filename.The output file's sample rate is specified by sample-rate. The output file header-format should be one of: AIFF, WAVE, NeXT. The output file sample-format should be one of: int16, int24, int32, float, double. The number of channels in the output file is specified with the -o argument.
...
| --- | --- || int | a 32 bit integer. | | float | a 32 bit single precision floating point number. | | double | a 64 bit double precision floating point number. | | string | an OSC format string: 8 bit ASCII characters, zero padded to a multiple of 4 bytes. | | bytes | a buffer of data preceded by a 32 bit length field and padded to a multiple of 4 bytes. | 
...
| 'i' | an int | 
| --- | --- || 'f' | a float | | 's' | a string | | 'b' | bytes | | 'd' | a double | | 't' | a timestamp | 
...
| --- | --- || string | a string with tags defined by the types of the arguments to follow. The tag string begins with a comma ',' character. | | ... | any combination of arguments of types: int, float, string or bytes. | 
...
**Open Sound Control**
: a protocol defined by CNMAT at UCBerkeley for controlling synthesizers. See [http://cnmat.cnmat.berkeley.edu/OSC/](http://cnmat.cnmat.berkeley.edu/OSC/) .
