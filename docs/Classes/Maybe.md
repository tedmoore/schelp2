...
**Related:** [Fdef](../Classes/Fdef.md), [Overviews/JITLib](../Overviews/JITLib.md)
...
+ Maybe {
    myMethod { |...args, kwargs|
        ^this.composeNAryOp(\myMethod, args, kwargs)
    }
}
```
...
a.source = 5;
a.value; // 5
a.source = { [1, 2, 3].rotate };
a.value; // [3, 2, 1]
a.source = a; // deliberately make recursion
a.value; // nil
Maybe.verbose = true;
...
b = Maybe();
c = a <> b;
c.value(1); // nil, not yet specified
b.source = { |x| x * 2 };
c.value(1);  // 1 * 2 + 1 = 3
```
