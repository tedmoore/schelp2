...
**Related:** [Stream](../Classes/Stream.md), [FilterPattern](../Classes/FilterPattern.md), [ListPattern](../Classes/ListPattern.md)
...
**Pattern** is an abstract class that is the base for the Patterns library. These classes form a rich and concise score language for music. The series of help files entitled [Streams-Patterns-Events1](../Tutorials/Streams-Patterns-Events1.md) gives a detailed introduction. This attempts a briefer characterization.
...
> **Note:** For reasons of efficiency, the implementation of `embedInStream` assumes that it is called from within a [Routine](../Classes/Routine.md). Consequently, `embedInStream` should never be called from within the function that defines a [FuncStream](../Classes/FuncStream.md) or a [Pfunc](../Classes/Pfunc.md) (the pattern that creates FuncStreams).




### Event Patterns
An [Event](../Classes/Event.md) is a [Environment](../Classes/Environment.md) with a 'play' method. Typically, an Event consists of a collection of key/value pairs that determine what the play method actually does. The values may be any object including functions defined in terms of other named attributes. Changing those values can generate a succession of sounds sometimes called 'music'... The pattern [Pbind](../Classes/Pbind.md) connects specific patterns with specific names. Consult its help page for details.
...
| `headerFormat` | File format, defaults to [server.recHeaderFormat](../Classes/Server.md#-recheaderformat) - see [SoundFile](../Classes/SoundFile.md) for supported header and sample formats. |  
| `sampleFormat` | Sample format, defaults to [server.recSampleFormat](../Classes/Server.md#-recsampleformat). |  
| `numChannels` | Number of channels to record, default 2. |  
| `dur` | How long to run the pattern before stopping. If nil (default), the pattern will run until it finishes on its own; then recording stops. Or, use cmd-period to stop the recording. If a number is given, the pattern will run for that many beats and then stop (using [Pfindur](../Classes/Pfindur.md)), ending the recording as well. |  
...
| `outNumChannels` | the number of out channels to play to the recorder. If larger than numChannels, outNumChannels will wrap . |  
