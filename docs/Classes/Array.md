...
**Related:** [Literals](../Reference/Literals.md), [List](../Classes/List.md)

**Categories:** Collections>Ordered

## Description

Arrays are ArrayedCollections whose slots may contain any object. Arrays have a fixed maximum size beyond which they cannot grow. For expandable arrays, use the [List](../Classes/List.md) class.
**Literal Arrays** can be created at compile time, and are very efficient. See [Literals](../Reference/Literals.md) for information.
...
This allows an efficient use of resources, only growing the array when it needs to. The [List](../Classes/List.md) class manages the Array internally, and in many cases is more suitable.


Items can be put into an existing slot with `a.put(2, obj)` and accessed with `a.at(2)` or `a[2]`.

> **Note:** Arrays use 0-based indexing, meaning the first item is accessed with index 0. Therefore, the index 2 in `a.put(2, obj)`, `a.at(2)`, and `a[2]` refers to the third item.


See [ArrayedCollection](../Classes/ArrayedCollection.md) for the principal methods: at, put, clipAt, wrapAt, etc.
