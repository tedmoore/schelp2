...
**Related:** [LocalIn](../Classes/LocalIn.md)

**Categories:** UGens>InOut

## Description

LocalOut writes to buses that are local to the enclosing synth. The buses should have been defined by a [LocalIn](../Classes/LocalIn.md) ugen. The `channelsArray` must be the same number of channels as were declared in the [LocalIn](../Classes/LocalIn.md) . These are like the global buses, but are more convenient if you want to implement a self contained effect that uses a feedback processing loop.
> **⚠️ Warning:** Audio written to a LocalOut will not be read by a corresponding [LocalIn](../Classes/LocalIn.md) until the next cycle, i.e. one block size of samples later. Because of this it is important to take this additional delay into account when using [LocalIn](../Classes/LocalIn.md) to create feedback delays with delay times shorter than the threshold of pitch (i. e. < 0.05 seconds or > 20Hz), or where sample accurate alignment is required. See the resonator example below.
