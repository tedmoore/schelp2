...
**Related:** [RecordBuf](../Classes/RecordBuf.md), [DiskIn](../Classes/DiskIn.md), [BufRd](../Classes/BufRd.md)
...
| `doneAction` | an integer representing an action to be executed when the buffer is finished playing. This can be used to free the enclosing synth, etc. See [Done](../Classes/Done.md) for more detail. `doneAction` is only evaluated if loop is 0. |  
