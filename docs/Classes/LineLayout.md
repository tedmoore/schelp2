...
If the item is a stretchable empty space (nil) the align argument will have no effect; if the item is a fixed-size empty space (an Integer) both stretch and align arguments will have no effect.### `insert`
...
| `stretch` | An integer stretch factor. |  
| `align` | A symbol expressing the alignment. See [gui_alignments](../Reference/gui_alignments.md) for possible values. |  
If the item is a stretchable empty space (nil) the align argument will have no effect; if the item is a fixed-size empty space (an Integer) both stretch and align arguments will have no effect.### `setStretch`
