...
**Related:** [Pattern](../Classes/Pattern.md), [Environment](../Classes/Environment.md), [IdentityDictionary](../Classes/IdentityDictionary.md)
...
### `addParentType`
Define an alternative parent that is chosen on the basis of **~type**. It allows you to change the defaults of an existing event type function.**Arguments:**
...
### `removeEventType`
Remove an event type function from the collection. If there is an associated event type parent, this will be removed also.**Arguments:**
...
## Basic Usage
Events can be written as a series of key value pairs enclosed in parentheses. Empty parentheses will create an empty event. They may be also used for object prototyping - see [Environment](../Classes/Environment.md) for more details.
...
Per default, the play message derives its behaviour from the [#defaultParentEvent](#defaultparentevent), which provides many default values, such as default instrument (\default), note (0), legato (0.8) and so on. Depending on the event type, these may differ completely and need not even represent a sound.
...
> **Note:** The use of [OffsetOut](../Classes/OffsetOut.md) in the SynthDef prevents irregularities that can result from the interaction of the timing of a sequence of notes and the control rate of the Server.
...
### Events and Patterns
Events are closely integrated with the Patterns library. Different patterns can be bound to different keys (or collections of keys) to specify the resultant music. See the help files [Pattern](../Classes/Pattern.md) and [Pbind](../Classes/Pbind.md) and the tutorials [Streams-Patterns-Events4](../Tutorials/Streams-Patterns-Events4.md) and [Streams-Patterns-Events5](../Tutorials/Streams-Patterns-Events5.md) for more details on Patterns.
...
Events also specify timing within a [Pattern](../Classes/Pattern.md). Event's `delta` method returns the value of `~delta` or, if that is nil, `~dur * ~stretch`.

Patterns are played by [TempoClock](../Classes/TempoClock.md)s, which have their own tempo controls. This tempo which can be controlled through `~tempo` in the event. Changes to the tempo affect everything else scheduled by the TempoClock, so `tempo` provides a global tempo control while `stretch` provides a control limited to the one pattern.
...
![Event-default-note.png](Event-default-note.png)
...
### Event types
An Event responds to a play message by evaluating `~play` in the event, which by default uses the event's type to define the action to be performed. See [Event_types](../Overviews/Event_types.md).
