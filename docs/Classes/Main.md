...
### `openPorts`
Get a collection of all active UDP ports, including the main sclang port `NetAddr.langPort`.**Returns:** A [Set](../Classes/Set.md).### `pid`
**Returns:** The operating system's pid (process ID) for the process.### `recompile`
Recompiles the class library.### `platform`
Get the current [Platform](../Classes/Platform.md)### `argv`
