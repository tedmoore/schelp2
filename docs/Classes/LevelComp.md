...
*Level compensation logic for Splay and SplayAz*

**Categories:** Panning

**Related:** [Splay](../Classes/Splay.md), [SplayAz](../Classes/SplayAz.md), [Level_Compensation](../Guides/Level_Compensation.md)

## Description

Mixing and Panning multichannel signals often requires [Level_Compensation](../Guides/Level_Compensation.md). LevelComp provides a logic for this that is used internally in the [Splay](../Classes/Splay.md) and [SplayAz](../Classes/SplayAz.md) UGens. Using the `levelComp` argument of `Splay/Az`, it calculates a level compensation factor applied to the input signals, depending on the number of input channels.
...
|----------|-------------|
| `levelComp` | a flag or numeric value:- `true` : equal power level compensation (default)
...
| `rate` | the rate for which to calculate - \audio or \control. |  
| `n` | the number of channels being mixed and scaled. |  
**Returns:** the levelComp factor by which to multiply the output signals.
## Examples
...
### Discussion and demonstration of level compensation options:
...
So for maximum peak safety, use `levelComp: 1` (or equivalently, set `levelComp: false`, and explicitly set level to 1/n, i.e. divide by the number of channels for amplitude compensation).
