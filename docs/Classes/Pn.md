...
| `repeats` | Repeats the enclosed pattern **repeats** times. |  
| `key` | If **key** is non-nil, it sets the value of that key to true whenever it restarts the pattern. This can be used to advance Patterns enclosed by [Pgate](../Classes/Pgate.md). |  
