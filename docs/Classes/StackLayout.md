...
### `count`
 The number of views managed by the layout.### `index`
 Sets or gets the index of the current view. The current view is placed on top of others, and if [#-mode](#-mode) is 0, all the others are hidden.### `mode`
