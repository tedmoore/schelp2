...
**Related:** [DynKlank](../Classes/DynKlank.md), [Klang](../Classes/Klang.md)
...
> **Note:** for dynamic changes of parameters refer to [DynKlank](../Classes/DynKlank.md)
...
Klank is a bank of Ringz filters. [Formlet](../Classes/Formlet.md) is equivalent to `Ringz.ar(... decay...) - Ring.ar(... attack...)`. Therefore, a more efficient way to make a bank of fixed-parameter Formlet filters is `Klank(`decaySpecs, ...) - Klank.ar(`attackSpecs, ...)` or `Klank.ar(`specs, ..., decayscale: decay) - Klank.ar(`specs, ..., decayscale: attack)`.
...
|----------|-------------|
| `specificationsArrayRef` | A [Ref](../Classes/Ref.md) to an [Array](../Classes/Array.md) of three Arrays:
**frequencies:**
...
The parameters in `specificationsArrayRef` can't be changed after it has been started. For a modulatable but less efficient version, see [DynKlank](../Classes/DynKlank.md).
## Examples

Four resonators each at maximum amplitude of 1.0 and ring times of 1 second, different exciters and no scaling:

> **Note:** Watch the ` before the opening bracket of the parameter array! Also see [Multichannel-Expansion](../Guides/Multichannel-Expansion.md)
