...
**Related:** [Pseg](../Classes/Pseg.md)

**Categories:** Streams-Patterns-Events>Patterns>Time

## Description

Pstep is a "sample and hold" pattern: The value returned by `next` is held at each value in `levels` for the corresponding duration in `durs`. This cycling is similar to [Ptuple](../Classes/Ptuple.md), in that the current repeat ends when eiher pattern returns nil.
...
See [Pseg](../Classes/Pseg.md) for a pattern whose value changes like an [Env](../Classes/Env.md).
...
|----------|-------------|
| `levels` | A number, collection, or [Pattern](../Classes/Pattern.md) that returns the levels. |  
| `durs` | A number, collection, or [Pattern](../Classes/Pattern.md) that returns segments durations in beats. |  
