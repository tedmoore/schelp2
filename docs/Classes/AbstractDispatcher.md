...
### `free`
[#-unregister](#-unregister) this dispatcher and remove it from [#*all](#*all). After this the dispatcher should be discarded.
### `typeKey`
Subclasses should override this method to return a key indicating the type of message this dispatcher responds to, e.g. `'OSC matched'` or `'MIDI control'`.**Returns:** A [Symbol](../Classes/Symbol.md).
### `update`
