...
### `size`
Return the number of elements the ArrayedCollection.### `maxSize`
...
### `plot`
Plot values in a GUI window. See [plot](../Reference/plot.md) for more details. When the receiver contains `nil` items, the plot fails with an error.### `swap`
...
### `grow`
Increase the size of the ArrayedCollection by **sizeIncrease** number of slots, possibly returning a new collection.### `growClear`
