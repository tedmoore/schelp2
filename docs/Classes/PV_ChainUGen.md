...
`PV_ChainUGen` is an abstract class, not used directly, but only its subclasses are. It represents phase-vocoder UGens - i.e. UGens which apply some kind of transformation to the frequency-domain signal produced by [FFT](../Classes/FFT.md).
It encompasses all unit generators whose output is an FFT chain. This is why [FFT](../Classes/FFT.md) is in this group but [IFFT](../Classes/IFFT.md) is not - the IFFT ugen outputs ordinary time-domain audio.
For more information on using these unit generators, see [FFT-Overview](../Guides/FFT-Overview.md).
