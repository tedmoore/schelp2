...
The **makeFunction** "makes" the protoEvent (i.e. protoEvent is `currentEnvironment`). Typically, it defines and yields a sequence of events that create the needed resources using the following [Event_types](../Overviews/Event_types.md):
...
The function yields each event. That event is then performed with possible modifications by enclosing patterns and the player (either an [EventStreamPlayer](../Classes/EventStreamPlayer.md) or a [ScoreStreamPlayer](../Classes/ScoreStreamPlayer.md)). The resultant event is returned to the function where it can be assigned to a key within the protoEvent.
