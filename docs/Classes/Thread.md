...
### `seconds`
Get or set the current logical time of the Thread in seconds.Setting `seconds` also sets [#-beats](#-beats) to `thisThread.clock.secs2beats(seconds)`.See [#-beats](#-beats) for general discussion on Threads and logical time.
### `clock`
