...
> **Note:** Within the function braces, the `|out|` argument defines a SynthDef *control input*, which is then used as the first input to `Out.ar`. It is a good habit to provide an `out` control in every SynthDef. Review [Getting-Started/04-Functions-and-Other-Functionality](../../Tutorials/Getting-Started/04-Functions-and-Other-Functionality.md) for more about function arguments.
...
Click here to go on to the next section: [Getting-Started/11-Busses](../../Tutorials/Getting-Started/11-Busses.md)
