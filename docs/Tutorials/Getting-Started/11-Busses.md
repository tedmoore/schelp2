...
The number of control and audio busses available, as well as the number of input and output channels, is set at the time the server app is booted. (See [ServerOptions](../../Classes/ServerOptions.md) for information on how to set the number of input and output channels, and busses.)
...
The truth is that there aren't multichannel busses per se, but Bus objects are able to represent a series of busses with consecutive indices. They *encapsulate* several adjacent server-side busses into a single Bus object, allowing you to treat them as a group. This turns out to be rather handy.
...
This is a complicated topic, and there are some exceptions to this, but you should be aware that ordering is crucial when interconnecting synths. The file [Order-of-execution](../../Guides/Order-of-execution.md) covers this topic in greater detail.
...
A target can be another Synth (or some other things; more on that soon), and an addAction is a symbol. See [Synth](../../Classes/Synth.md) for a complete list of possible addActions.
...
[Bus](../../Classes/Bus.md), [In](../../Classes/In.md), [OutputProxy](../../Classes/OutputProxy.md), [Order-of-execution](../../Guides/Order-of-execution.md), [Synth](../../Classes/Synth.md)
...
Click here to go on to the next section: [Getting-Started/12-Groups](../../Tutorials/Getting-Started/12-Groups.md)
