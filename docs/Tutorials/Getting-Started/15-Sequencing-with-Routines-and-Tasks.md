...
Note that routines are used for the data, but task is used for play. Also, unlike the previous infinite sequences, this one stops when it runs out of data. That's the purpose of the while loop -- it continues only as long as the 'dur' data stream keeps pumping out values. (See the [Conditional-Execution](../../Reference/Conditional-Execution.md) helpfile for more on **while**.)
...
Latency works by adding itself to the current time on the clock. If all the messages go out with the same latency value, their timing will be precise relative to each other and to the clock. The [ServerTiming](../../Guides/ServerTiming.md) help file explains in more detail how this works, but you don't really need to know all of that in order to use it. The main point is to use a consistent, small latency value for perfect timing. (A Server object has a latency variable that you can use for consistency.)
...
[Routine](../../Classes/Routine.md), [Task](../../Classes/Task.md), [Quant](../../Classes/Quant.md), [ServerTiming](../../Guides/ServerTiming.md), [Bundled-Messages](../../Guides/Bundled-Messages.md)
...
Click here to go on to the next section: [Getting-Started/16-Sequencing-with-Patterns](../../Tutorials/Getting-Started/16-Sequencing-with-Patterns.md)
