...
> **Note:** By default the interpreter variable `s` is set to the local server at startup. For further information see the [Server](../Classes/Server.md) helpfile.
...
## Making Sound
(note: This tutorial uses raw OSC commands as described in [Server-Command-Reference](../Reference/Server-Command-Reference.md), rather than the classes [Synth](../Classes/Synth.md) and [Group](../Classes/Group.md). See those helpfiles also for some simpler ways of working with Synths. This tutorial explains the basic underlying design of Synths and SynthDefs).
