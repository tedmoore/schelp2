...
## Introduction
The most direct and fast way to send commands to the server is to send messages to the [Server](../Classes/Server.md) object, if you are within sc-lang. If you are in a shell you can use sendOSC (available from CNMAT).

this messaging scheme is explained in detail in:

- [Server-Architecture](../Reference/Server-Architecture.md)
- [Server-Command-Reference](../Reference/Server-Command-Reference.md)
- [Server_Tutorial](../Tutorials/Server_Tutorial.md)
...
As soon as you want to pass around the reference to a certain node, assuming that you might not have only one server, it can be useful to create a [Synth](../Classes/Synth.md) or [Group](../Classes/Group.md) object. These objects also respond to messages, and when needed can be used to obtain the state of the server side node.

see [Node](../Classes/Node.md), [Synth](../Classes/Synth.md), and [Group](../Classes/Group.md) help for more detailed helpfiles on node objects.
...
In cases where you need to keep track of the synth's state, it is advisable to use node objects and register them with a [NodeWatcher](../Classes/NodeWatcher.md). (see helpfile)
