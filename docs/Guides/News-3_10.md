...
*All the news that's fit to .postln*
...
Providing an integer index for `SerialPort.new` is deprecated.



## Class library: Fixed
`BufWr.ar` no longer allows its input signals to be control rate, which caused the server to read from garbage memory.
