...
**Related:** [NetAddr](../Classes/NetAddr.md), [OSCFunc](../Classes/OSCFunc.md)

OSC communication between programs is often done to send messages from one application to another, possibly with the applications running on different computers. In SuperCollider this communication is done by creating a [NetAddr](../Classes/NetAddr.md) of the target application and creating an [OSCFunc](../Classes/OSCFunc.md) to listen to another application. The underlying protocol of OSC is either UDP or TCP.
...
> **Note:** The port 7771 above is the port the other application is sending **from**, not the port SC is receiving on. See the **recvPort** argument in [OSCFunc#*new](../Classes/OSCFunc.md#*new) if you want to receive on another port than NetAddr.langPort.
